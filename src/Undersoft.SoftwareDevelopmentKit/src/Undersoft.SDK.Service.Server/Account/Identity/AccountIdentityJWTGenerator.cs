...
public class AccountIdentityJWTGenerator
{
    AccountIdentityJWTOptions options;
    JwtSecurityTokenHandler handler;
    TokenValidationParameters validationParameters;
    SecurityKey securitykey;

    public AccountIdentityJWTGenerator(Action<AccountIdentityJWTOptions> builder = null) : this(0, null)
    {
        options = new AccountIdentityJWTOptions();
        if (builder != null)
            builder(options);
    }

    public AccountIdentityJWTGenerator(int minutesToExpire, AccountIdentityJWTOptions jwtOptions = null)
    {
        options = new AccountIdentityJWTOptions();
        if (jwtOptions != null)
            options = jwtOptions;
