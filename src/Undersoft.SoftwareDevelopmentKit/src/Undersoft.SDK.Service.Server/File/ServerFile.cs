...
{
    public class ServerFile : DataFile, IFormFile, IServerFile
    {
        private IFormFile _formFile;
        private Stream _stream;

        public ServerFile(FileContainer container, string filename) : base(container.ContainerName)
        {
        ...
        public ServerFile(string containerName, string filename) : base(containerName)
        {
            var task = GetOrNullAsync(filename);
            task.Wait();
            _stream = task.Result;
        ...
        public ServerFile(string path) : base(Path.GetDirectoryName(path))
        {
