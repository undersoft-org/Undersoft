namespace Undersoft.SDK.Security.Cryptography
{
    using System.Security.Cryptography;
    using System.Text;

    internal class CryptoBlowFish
    {
        ...
        public byte[] Crypt_CTR(byte[] text, int numThreads)
        {
            ...
            for (int i = 8; i < plainText.Length; i += 8)
            {
                for (int x = 0; x < 8; x++)
                {
        ...
        public string Decrypt_CBC(string ct)
        {
            ...
            return Encoding.ASCII
                .GetString(Decrypt_CBC(HexToByte(ct.Substring(16))))
                .Replace("\0", "");
        }

        public byte[] Decrypt_CTR(byte[] ct)
        {
            for (int i = 0; i < 8; i++)
                IV[i] = ct[i];
            return Crypt_CTR(ct, 2);
        }
        ...
        public string Decrypt_CTR(string ct)
        {
            IV = HexToByte(ct.Substring(0, 16));
            return Encoding.ASCII
                .GetString(Crypt_CTR(HexToByte(ct.Substring(16)), 2))
                .Replace("\0", "");
        }
        ...
        public byte[] Encrypt_CTR(byte[] pt)
        {
            if (!IVSet)
                SetRandomIV();
            byte[] crypt = Crypt_CTR(pt, 2);
            InitVector.CopyTo(crypt, 0);
            return crypt;
        }
        ...
        private void SetupKey(byte[] cipherKey)
        {
            ...
            {
                throw new Exception("Id too long. 56 bytes required.");
            }
            ...
            for (int i = 0; i < 18; i++)
            {
                uint d = (uint)(
                    (
                        (key[j % cipherKey.Length] * 256 + key[(j + 1) % cipherKey.Length]) * 256
                        + key[(j + 2) % cipherKey.Length]
                    ) * 256
