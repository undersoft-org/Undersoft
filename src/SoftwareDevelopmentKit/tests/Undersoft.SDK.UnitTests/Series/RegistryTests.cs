...
/// <summary>
/// Unit tests for the type <see cref="Registry"/>.
/// </summary>
public class Registry_1Tests
{
    private Registry<V> _testClass;
    ...
    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="Registry"/>.
    /// </summary>
    public Registry_1Tests()
    {
        ...
        this._testClass = _fixture.Create<Registry<V>>();
    }
    ...
    {
        // Act
        var instance = new Registry<V>(this._collection, this._capacity, this._repeatable);

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new Registry<V>(this._repeatable, this._capacity);
    ...
    {
        FluentActions.Invoking(() => new Registry<V>(default(IEnumerable<V>), this._capacity, this._repeatable)).Should().Throw<ArgumentNullException>().WithParameterName("collection");
    }
