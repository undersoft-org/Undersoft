...
/// <summary>
/// Unit tests for the type <see cref="TypedCatalog"/>.
/// </summary>
public class TypedCatalog_1Tests
{
    private TypedCatalog<V> _testClass;
    ...
    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="TypedCatalog"/>.
    /// </summary>
    public TypedCatalog_1Tests()
    {
        ...
        this._testClass = _fixture.Create<TypedCatalog<V>>();
    }
    ...
    {
        // Act
        var instance = new TypedCatalog<V>(this._collectionIEnumerableISeriesItemV, this._capacity, this._bits);

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new TypedCatalog<V>(this._collectionIListISeriesItemV, this._capacity, this._bits);

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new TypedCatalog<V>(this._capacity, this._bits);
    ...
    {
        FluentActions.Invoking(() => new TypedCatalog<V>(default(IEnumerable<ISeriesItem<V>>), this._capacity, this._bits)).Should().Throw<ArgumentNullException>().WithParameterName("collection");
        FluentActions.Invoking(() => new TypedCatalog<V>(default(IList<ISeriesItem<V>>), this._capacity, this._bits)).Should().Throw<ArgumentNullException>().WithParameterName("collection");
    }
