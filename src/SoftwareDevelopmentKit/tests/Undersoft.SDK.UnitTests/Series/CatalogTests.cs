...
/// <summary>
/// Unit tests for the type <see cref="Catalog"/>.
/// </summary>
public class Catalog_1Tests
{
    private Catalog<V> _testClass;
    ...
    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="Catalog"/>.
    /// </summary>
    public Catalog_1Tests()
    {
        ...
        this._testClass = _fixture.Create<Catalog<V>>();
    }
    ...
    {
        // Act
        var instance = new Catalog<V>(this._collectionIEnumerableISeriesItemV, this._capacity, this._bits);

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new Catalog<V>(this._collectionIListISeriesItemV, this._capacity, this._bits);

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new Catalog<V>(this._collectionIEnumerableV, this._capacity, this._bits);

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new Catalog<V>(this._capacity, this._bits);
    ...
    {
        FluentActions.Invoking(() => new Catalog<V>(default(IEnumerable<ISeriesItem<V>>), this._capacity, this._bits)).Should().Throw<ArgumentNullException>().WithParameterName("collection");
        FluentActions.Invoking(() => new Catalog<V>(default(IList<ISeriesItem<V>>), this._capacity, this._bits)).Should().Throw<ArgumentNullException>().WithParameterName("collection");
        FluentActions.Invoking(() => new Catalog<V>(default(IEnumerable<V>), this._capacity, this._bits)).Should().Throw<ArgumentNullException>().WithParameterName("collection");
    }
