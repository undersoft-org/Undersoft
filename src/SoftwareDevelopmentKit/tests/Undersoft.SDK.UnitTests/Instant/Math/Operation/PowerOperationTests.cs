...
/// <summary>
/// Unit tests for the type <see cref="PowerOperation"/>.
/// </summary>
public class PowerOperationTests
{
    private PowerOperation _testClass;
    private IFixture _fixture;
    private Formula _e1;
    private Formula _e2;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="PowerOperation"/>.
    /// </summary>
    public PowerOperationTests()
    {
        _fixture = new Fixture().Customize(new AutoMoqCustomization());
        this._e1 = _fixture.Create<Formula>();
        this._e2 = _fixture.Create<Formula>();
        this._testClass = _fixture.Create<PowerOperation>();
    }
    ...
    {
        // Act
        var instance = new PowerOperation(this._e1, this._e2);
    ...
    {
        FluentActions.Invoking(() => new PowerOperation(default(Formula), this._e2)).Should().Throw<ArgumentNullException>().WithParameterName("e1");
    }
    ...
    {
        FluentActions.Invoking(() => new PowerOperation(this._e1, default(Formula))).Should().Throw<ArgumentNullException>().WithParameterName("e2");
    }
