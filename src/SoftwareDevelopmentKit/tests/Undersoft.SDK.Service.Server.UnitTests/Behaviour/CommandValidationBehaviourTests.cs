...
/// <summary>
/// Unit tests for the type <see cref="CommandValidationBehaviour"/>.
/// </summary>
[TestClass]
public class CommandValidationBehaviour_2Tests
{
    private CommandValidationBehaviour<TRequest, TResponse> _testClass;
    private IFixture _fixture;
    private IEnumerable<IValidator<TResponse>> _validators;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="CommandValidationBehaviour"/>.
    /// </summary>
    [TestInitialize]
    public void SetUp()
    {
        ...
        this._testClass = _fixture.Create<CommandValidationBehaviour<TRequest, TResponse>>();
    }
    ...
    {
        // Act
        var instance = new CommandValidationBehaviour<TRequest, TResponse>(this._validators);
    ...
    {
        FluentActions.Invoking(() => new CommandValidationBehaviour<TRequest, TResponse>(default(IEnumerable<IValidator<TResponse>>))).Should().Throw<ArgumentNullException>().WithParameterName("validators");
    }
