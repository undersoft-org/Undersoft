...
/// <summary>
/// Unit tests for the type <see cref="StreamDataController"/>.
/// </summary>
[TestClass]
public class StreamDataController_5Tests
{
    private class TestStreamDataController : StreamDataController<TKey, TEntry, TReport, TEntity, TDto>
    {
        public TestStreamDataController() : base()
        {
        }

        public TestStreamDataController(IServicer servicer) : base(servicer)
        {
        }

        public TestStreamDataController(IServicer servicer,
    ...
    private TestStreamDataController _testClass;
    ...
    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="StreamDataController"/>.
    /// </summary>
    [TestInitialize]
    public void SetUp()
    {
        ...
        this._testClass = _fixture.Create<TestStreamDataController>();
    }
    ...
    {
        // Act
        var instance = new TestStreamDataController();

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new TestStreamDataController(this._servicer.Object);

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new TestStreamDataController(this._servicer.Object, this._predicate, this._keysetter, this._keymatcher, this._publishMode);
    ...
    {
        FluentActions.Invoking(() => new TestStreamDataController(default(IServicer))).Should().Throw<ArgumentNullException>().WithParameterName("servicer");
        FluentActions.Invoking(() => new TestStreamDataController(default(IServicer), this._predicate, this._keysetter, this._keymatcher, this._publishMode)).Should().Throw<ArgumentNullException>().WithParameterName("servicer");
    }
    ...
    {
        FluentActions.Invoking(() => new TestStreamDataController(this._servicer.Object, default(Func<TDto, Expression<Func<TEntity, bool>>>), this._keysetter, this._keymatcher, this._publishMode)).Should().Throw<ArgumentNullException>().WithParameterName("predicate");
    }
    ...
    {
        FluentActions.Invoking(() => new TestStreamDataController(this._servicer.Object, this._predicate, default(Func<TKey, Func<TDto, object>>), this._keymatcher, this._publishMode)).Should().Throw<ArgumentNullException>().WithParameterName("keysetter");
    }
    ...
    {
        FluentActions.Invoking(() => new TestStreamDataController(this._servicer.Object, this._predicate, this._keysetter, default(Func<TKey, Expression<Func<TEntity, bool>>>), this._publishMode)).Should().Throw<ArgumentNullException>().WithParameterName("keymatcher");
    }
    ...
    {
        // Act
        var result = await ((IStreamDataController<TDto>)this._testClass).Count();
