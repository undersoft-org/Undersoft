...
/// <summary>
/// Unit tests for the type <see cref="ApiServiceController"/>.
/// </summary>
[TestClass]
public class ApiServiceController_3Tests
{
    private class TestApiServiceController : ApiServiceController<TStore, TService, TModel>
    {
        public TestApiServiceController() : base()
        {
        }

        public TestApiServiceController(
        IServicer servicer
    ) : base(servicer
)
        {
    ...
    private TestApiServiceController _testClass;
    private IFixture _fixture;
    private Mock<IServicer> _servicer;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="ApiServiceController"/>.
    /// </summary>
    [TestInitialize]
    public void SetUp()
    {
        ...
        this._testClass = _fixture.Create<TestApiServiceController>();
    }
    ...
    {
        // Act
        var instance = new TestApiServiceController();

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new TestApiServiceController(this._servicer.Object);
    ...
    {
        FluentActions.Invoking(() => new TestApiServiceController(default(IServicer))).Should().Throw<ArgumentNullException>().WithParameterName("servicer");
    }
