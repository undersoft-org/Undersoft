...
/// <summary>
/// Unit tests for the type <see cref="CommandValidatorBase"/>.
/// </summary>
public class CommandValidatorBase_1Tests
{
    private class TestCommandValidatorBase : CommandValidatorBase<TCommand>
    {
        public TestCommandValidatorBase(IServicer ultimateService) : base(ultimateService)
        {
        }
    ...
    private TestCommandValidatorBase _testClass;
    private IFixture _fixture;
    private Mock<IServicer> _ultimateService;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="CommandValidatorBase"/>.
    /// </summary>
    public CommandValidatorBase_1Tests()
    {
        _fixture = new Fixture().Customize(new AutoMoqCustomization());
        this._ultimateService = _fixture.Freeze<Mock<IServicer>>();
        this._testClass = _fixture.Create<TestCommandValidatorBase>();
    }
    ...
    {
        FluentActions.Invoking(() => new TestCommandValidatorBase(default(IServicer))).Should().Throw<ArgumentNullException>().WithParameterName("ultimateService");
    }
