...
/// <summary>
/// Unit tests for the type <see cref="CommandSetValidator"/>.
/// </summary>
public class CommandSetValidator_1Tests
{
    private class TestCommandSetValidator : CommandSetValidator<TDto>
    {
        public TestCommandSetValidator(IServicer ultimateService) : base(ultimateService)
        {
        }
        ...
        public void PublicValidateLength(int min, int max, Expression<Func<Command<TDto>, object>>[] members)
        {
            base.ValidateLength(min, max, members
);
        }

        public void PublicValidateCount(int min, int max, Expression<Func<Command<TDto>, object>>[] members)
        {
        ...
        public void PublicValidateExist<TStore, TEntity>(Func<TDto, Expression<Func<TEntity, bool>>> command, string message)
        {
            base.ValidateExist<TStore, TEntity>(command, message);
        }

        public void PublicValidateNotExist<TStore, TEntity>(Func<TDto, Expression<Func<TEntity, bool>>> command, string message)
        {
            base.ValidateNotExist<TStore, TEntity>(command, message);
        }
    }

    private TestCommandSetValidator _testClass;
    private IFixture _fixture;
    private Mock<IServicer> _ultimateService;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="CommandSetValidator"/>.
    /// </summary>
    public CommandSetValidator_1Tests()
    {
        _fixture = new Fixture().Customize(new AutoMoqCustomization());
        this._ultimateService = _fixture.Freeze<Mock<IServicer>>();
        this._testClass = _fixture.Create<TestCommandSetValidator>();
    }
    ...
    {
        // Act
        var instance = new TestCommandSetValidator(this._ultimateService.Object);
    ...
    {
        FluentActions.Invoking(() => new TestCommandSetValidator(default(IServicer))).Should().Throw<ArgumentNullException>().WithParameterName("ultimateService");
    }
    ...
    {
        FluentActions.Invoking(() => this._testClass.PublicValidateLength(_fixture.Create<int>(), _fixture.Create<int>(), default(Expression<Func<Command<TDto>, object>>[]))).Should().Throw<ArgumentNullException>().WithParameterName("members");
    }
    ...
    {
        FluentActions.Invoking(() => this._testClass.PublicValidateCount(_fixture.Create<int>(), _fixture.Create<int>(), default(Expression<Func<Command<TDto>, object>>[]))).Should().Throw<ArgumentNullException>().WithParameterName("members");
    }
    ...
    public void CanCallValidateExist()
    {
        ...
        // Act
        this._testClass.PublicValidateExist<TStore, TEntity>(command, message);
    ...
    {
        FluentActions.Invoking(() => this._testClass.PublicValidateExist<TStore, TEntity>(default(Func<TDto, Expression<Func<TEntity, bool>>>), _fixture.Create<string>())).Should().Throw<ArgumentNullException>().WithParameterName("command");
    }
    ...
    {
        FluentActions.Invoking(() => this._testClass.PublicValidateExist<TStore, TEntity>(x => _fixture.Create<Expression<Func<TEntity, bool>>>(), value)).Should().Throw<ArgumentNullException>().WithParameterName("message");
    }
    ...
    public void CanCallValidateNotExist()
    {
        ...
        // Act
        this._testClass.PublicValidateNotExist<TStore, TEntity>(command, message);
    ...
    {
        FluentActions.Invoking(() => this._testClass.PublicValidateNotExist<TStore, TEntity>(default(Func<TDto, Expression<Func<TEntity, bool>>>), _fixture.Create<string>())).Should().Throw<ArgumentNullException>().WithParameterName("command");
    }
    ...
    {
        FluentActions.Invoking(() => this._testClass.PublicValidateNotExist<TStore, TEntity>(x => _fixture.Create<Expression<Func<TEntity, bool>>>(), value)).Should().Throw<ArgumentNullException>().WithParameterName("message");
    }
}