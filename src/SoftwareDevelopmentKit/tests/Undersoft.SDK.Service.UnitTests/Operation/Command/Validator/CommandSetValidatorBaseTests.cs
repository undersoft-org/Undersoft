...
/// <summary>
/// Unit tests for the type <see cref="CommandSetValidatorBase"/>.
/// </summary>
public class CommandSetValidatorBase_1Tests
{
    private class TestCommandSetValidatorBase : CommandSetValidatorBase<TCommand>
    {
        public TestCommandSetValidatorBase(IServicer servicer) : base(servicer)
        {
        }
    ...
    private TestCommandSetValidatorBase _testClass;
    private IFixture _fixture;
    private Mock<IServicer> _servicer;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="CommandSetValidatorBase"/>.
    /// </summary>
    public CommandSetValidatorBase_1Tests()
    {
        _fixture = new Fixture().Customize(new AutoMoqCustomization());
        this._servicer = _fixture.Freeze<Mock<IServicer>>();
        this._testClass = _fixture.Create<TestCommandSetValidatorBase>();
    }
    ...
    {
        FluentActions.Invoking(() => new TestCommandSetValidatorBase(default(IServicer))).Should().Throw<ArgumentNullException>().WithParameterName("servicer");
    }
