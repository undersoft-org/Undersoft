...
/// <summary>
/// Unit tests for the type <see cref="Upserted"/>.
/// </summary>
public class Upserted_3Tests
{
    private Upserted<TStore, TEntity, TDto> _testClass;
    private IFixture _fixture;
    private Upsert<TStore, TEntity, TDto> _command;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="Upserted"/>.
    /// </summary>
    public Upserted_3Tests()
    {
        _fixture = new Fixture().Customize(new AutoMoqCustomization());
        this._command = _fixture.Create<Upsert<TStore, TEntity, TDto>>();
        this._testClass = _fixture.Create<Upserted<TStore, TEntity, TDto>>();
    }
    ...
    {
        // Act
        var instance = new Upserted<TStore, TEntity, TDto>(this._command);
    ...
    {
        FluentActions.Invoking(() => new Upserted<TStore, TEntity, TDto>(default(Upsert<TStore, TEntity, TDto>))).Should().Throw<ArgumentNullException>().WithParameterName("command");
    }
