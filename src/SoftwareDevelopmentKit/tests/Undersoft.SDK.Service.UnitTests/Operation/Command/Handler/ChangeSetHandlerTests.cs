...
/// <summary>
/// Unit tests for the type <see cref="ChangeSetHandler"/>.
/// </summary>
public class ChangeSetHandler_3Tests
{
    private ChangeSetHandler<TStore, TEntity, TDto> _testClass;
    private IFixture _fixture;
    private Mock<IServicer> _servicer;
    private Mock<IStoreRepository<TStore, TEntity>> _repository;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="ChangeSetHandler"/>.
    /// </summary>
    public ChangeSetHandler_3Tests()
    {
        ...
        this._testClass = _fixture.Create<ChangeSetHandler<TStore, TEntity, TDto>>();
    }
    ...
    {
        // Act
        var instance = new ChangeSetHandler<TStore, TEntity, TDto>(this._servicer.Object, this._repository.Object);
    ...
    {
        FluentActions.Invoking(() => new ChangeSetHandler<TStore, TEntity, TDto>(default(IServicer), this._repository.Object)).Should().Throw<ArgumentNullException>().WithParameterName("servicer");
    }
    ...
    {
        FluentActions.Invoking(() => new ChangeSetHandler<TStore, TEntity, TDto>(this._servicer.Object, default(IStoreRepository<TStore, TEntity>))).Should().Throw<ArgumentNullException>().WithParameterName("repository");
    }
    ...
    {
        // Arrange
        var request = _fixture.Create<ChangeSet<TStore, TEntity, TDto>>();
    ...
    {
        await FluentActions.Invoking(() => this._testClass.Handle(default(ChangeSet<TStore, TEntity, TDto>), _fixture.Create<CancellationToken>())).Should().ThrowAsync<ArgumentNullException>().WithParameterName("request");
    }
}