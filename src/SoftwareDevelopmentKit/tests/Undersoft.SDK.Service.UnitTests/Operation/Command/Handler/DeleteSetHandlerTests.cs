...
/// <summary>
/// Unit tests for the type <see cref="DeleteSetHandler"/>.
/// </summary>
public class DeleteSetHandler_3Tests
{
    private DeleteSetHandler<TStore, TEntity, TDto> _testClass;
    private IFixture _fixture;
    private Mock<IServicer> _servicer;
    private Mock<IStoreRepository<TStore, TEntity>> _repository;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="DeleteSetHandler"/>.
    /// </summary>
    public DeleteSetHandler_3Tests()
    {
        ...
        this._testClass = _fixture.Create<DeleteSetHandler<TStore, TEntity, TDto>>();
    }
    ...
    {
        // Act
        var instance = new DeleteSetHandler<TStore, TEntity, TDto>(this._servicer.Object, this._repository.Object);
    ...
    {
        FluentActions.Invoking(() => new DeleteSetHandler<TStore, TEntity, TDto>(default(IServicer), this._repository.Object)).Should().Throw<ArgumentNullException>().WithParameterName("servicer");
    }
    ...
    {
        FluentActions.Invoking(() => new DeleteSetHandler<TStore, TEntity, TDto>(this._servicer.Object, default(IStoreRepository<TStore, TEntity>))).Should().Throw<ArgumentNullException>().WithParameterName("repository");
    }
    ...
    {
        // Arrange
        var request = _fixture.Create<DeleteSet<TStore, TEntity, TDto>>();
    ...
    {
        await FluentActions.Invoking(() => this._testClass.Handle(default(DeleteSet<TStore, TEntity, TDto>), _fixture.Create<CancellationToken>())).Should().ThrowAsync<ArgumentNullException>().WithParameterName("request");
    }
}