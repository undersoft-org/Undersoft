...
/// <summary>
/// Unit tests for the type <see cref="ActionHandler"/>.
/// </summary>
public class ActionHandler_3Tests
{
    private ActionHandler<TStore, TService, TDto> _testClass;
    private IFixture _fixture;
    private Mock<IServicer> _servicer;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="ActionHandler"/>.
    /// </summary>
    public ActionHandler_3Tests()
    {
        _fixture = new Fixture().Customize(new AutoMoqCustomization());
        this._servicer = _fixture.Freeze<Mock<IServicer>>();
        this._testClass = _fixture.Create<ActionHandler<TStore, TService, TDto>>();
    }
    ...
    {
        // Act
        var instance = new ActionHandler<TStore, TService, TDto>(this._servicer.Object);
    ...
    {
        FluentActions.Invoking(() => new ActionHandler<TStore, TService, TDto>(default(IServicer))).Should().Throw<ArgumentNullException>().WithParameterName("servicer");
    }
