...
/// <summary>
/// Unit tests for the type <see cref="RemoteUpdated"/>.
/// </summary>
public class RemoteUpdated_3Tests
{
    private RemoteUpdated<TStore, TDto, TModel> _testClass;
    private IFixture _fixture;
    private RemoteUpdate<TStore, TDto, TModel> _command;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="RemoteUpdated"/>.
    /// </summary>
    public RemoteUpdated_3Tests()
    {
        _fixture = new Fixture().Customize(new AutoMoqCustomization());
        this._command = _fixture.Create<RemoteUpdate<TStore, TDto, TModel>>();
        this._testClass = _fixture.Create<RemoteUpdated<TStore, TDto, TModel>>();
    }
    ...
    {
        // Act
        var instance = new RemoteUpdated<TStore, TDto, TModel>(this._command);
    ...
    {
        FluentActions.Invoking(() => new RemoteUpdated<TStore, TDto, TModel>(default(RemoteUpdate<TStore, TDto, TModel>))).Should().Throw<ArgumentNullException>().WithParameterName("command");
    }
