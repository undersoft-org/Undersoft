...
/// <summary>
/// Unit tests for the type <see cref="RemoteCommandBase"/>.
/// </summary>
public class RemoteCommandBaseTests
{
    private class TestRemoteCommandBase : RemoteCommandBase
    {
        public TestRemoteCommandBase() : base()
        {
        }

        public TestRemoteCommandBase(OperationType commandMode, EventPublishMode publishMode) : base(commandMode, publishMode)
        {
        }

        public TestRemoteCommandBase(object entryData, OperationType commandMode, EventPublishMode publishMode) : base(entryData, commandMode, publishMode)
        {
        }

        public TestRemoteCommandBase(
                                                                                                                        object entryData,
    ...
    private TestRemoteCommandBase _testClass;
    ...
    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="RemoteCommandBase"/>.
    /// </summary>
    public RemoteCommandBaseTests()
    {
        ...
        this._testClass = _fixture.Create<TestRemoteCommandBase>();
    }
    ...
    {
        // Act
        var instance = new TestRemoteCommandBase();

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new TestRemoteCommandBase(this._commandMode, this._publishMode);

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new TestRemoteCommandBase(this._entryData, this._commandMode, this._publishMode);

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new TestRemoteCommandBase(this._entryData, this._commandMode, this._publishMode, this._keys);

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new TestRemoteCommandBase(this._commandMode, this._publishMode, this._keys);
    ...
    {
        FluentActions.Invoking(() => new TestRemoteCommandBase(default(object), this._commandMode, this._publishMode)).Should().Throw<ArgumentNullException>().WithParameterName("entryData");
        FluentActions.Invoking(() => new TestRemoteCommandBase(default(object), this._commandMode, this._publishMode, this._keys)).Should().Throw<ArgumentNullException>().WithParameterName("entryData");
    }
    ...
    {
        FluentActions.Invoking(() => new TestRemoteCommandBase(this._entryData, this._commandMode, this._publishMode, default(object[]))).Should().Throw<ArgumentNullException>().WithParameterName("keys");
        FluentActions.Invoking(() => new TestRemoteCommandBase(this._commandMode, this._publishMode, default(object[]))).Should().Throw<ArgumentNullException>().WithParameterName("keys");
    }
    ...
    /// <summary>
    /// Checks that the Model property can be read from and written to.
    /// </summary>
    [TestMethod]
    public void CanSetAndGetModel()
    {
        ...
        // Act
        this._testClass.Model = testValue;

        // Assert
        this._testClass.Model.Should().BeSameAs(testValue);
    }
