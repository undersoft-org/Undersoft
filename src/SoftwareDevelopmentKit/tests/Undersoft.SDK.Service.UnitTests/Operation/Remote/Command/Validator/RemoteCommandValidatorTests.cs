...
/// <summary>
/// Unit tests for the type <see cref="RemoteCommandValidator"/>.
/// </summary>
public class RemoteCommandValidator_1Tests
{
    private class TestRemoteCommandValidator : RemoteCommandValidator<TModel>
    {
        public TestRemoteCommandValidator(IServicer ultimateService) : base(ultimateService)
        {
        }
        ...
        public void PublicValidateLength(int min, int max, Expression<Func<RemoteCommand<TModel>, string>>[] members)
        {
            base.ValidateLength(min, max, members
);
        }

        public void PublicValidateCount(int min, int max, Expression<Func<RemoteCommand<TModel>, ICollection>>[] members)
        {
        ...
        public void PublicValidateExist<TStore, TEntity>(Func<TModel, Expression<Func<TEntity, bool>>> command)
        {
            base.ValidateExist<TStore, TEntity>(command
);
        }

        public void PublicValidateNotExist<TStore, TEntity>(Func<TModel, Expression<Func<TEntity, bool>>> command)
        {
    ...
    private TestRemoteCommandValidator _testClass;
    private IFixture _fixture;
    private Mock<IServicer> _ultimateService;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="RemoteCommandValidator"/>.
    /// </summary>
    public RemoteCommandValidator_1Tests()
    {
        _fixture = new Fixture().Customize(new AutoMoqCustomization());
        this._ultimateService = _fixture.Freeze<Mock<IServicer>>();
        this._testClass = _fixture.Create<TestRemoteCommandValidator>();
    }
    ...
    {
        // Act
        var instance = new TestRemoteCommandValidator(this._ultimateService.Object);
    ...
    {
        FluentActions.Invoking(() => new TestRemoteCommandValidator(default(IServicer))).Should().Throw<ArgumentNullException>().WithParameterName("ultimateService");
    }
    ...
    {
        FluentActions.Invoking(() => this._testClass.PublicValidateLength(_fixture.Create<int>(), _fixture.Create<int>(), default(Expression<Func<RemoteCommand<TModel>, string>>[]))).Should().Throw<ArgumentNullException>().WithParameterName("members");
    }
    ...
    public void CanCallValidateCount()
    {
        ...
        var min = _fixture.Create<int>();
        var max = _fixture.Create<int>();
        var members = _fixture.Create<Expression<Func<RemoteCommand<TModel>, ICollection>>[]>();
    ...
    {
        FluentActions.Invoking(() => this._testClass.PublicValidateCount(_fixture.Create<int>(), _fixture.Create<int>(), default(Expression<Func<RemoteCommand<TModel>, ICollection>>[]))).Should().Throw<ArgumentNullException>().WithParameterName("members");
    }
    ...
    {
        // Arrange
        Func<TModel, Expression<Func<TEntity, bool>>> command = x => _fixture.Create<Expression<Func<TEntity, bool>>>();
    ...
    {
        FluentActions.Invoking(() => this._testClass.PublicValidateExist<TStore, TEntity>(default(Func<TModel, Expression<Func<TEntity, bool>>>))).Should().Throw<ArgumentNullException>().WithParameterName("command");
    }
    ...
    {
        FluentActions.Invoking(() => this._testClass.PublicValidateNotExist<TStore, TEntity>(default(Func<TModel, Expression<Func<TEntity, bool>>>))).Should().Throw<ArgumentNullException>().WithParameterName("command");
    }
}