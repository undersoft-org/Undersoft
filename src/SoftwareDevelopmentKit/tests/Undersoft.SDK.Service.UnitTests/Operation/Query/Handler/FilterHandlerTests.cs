...
/// <summary>
/// Unit tests for the type <see cref="FilterHandler"/>.
/// </summary>
public class FilterHandler_3Tests
{
    private FilterHandler<TStore, TEntity, TDto> _testClass;
    private IFixture _fixture;
    private Mock<IStoreRepository<TStore, TEntity>> _repository;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="FilterHandler"/>.
    /// </summary>
    public FilterHandler_3Tests()
    {
        _fixture = new Fixture().Customize(new AutoMoqCustomization());
        this._repository = _fixture.Freeze<Mock<IStoreRepository<TStore, TEntity>>>();
        this._testClass = _fixture.Create<FilterHandler<TStore, TEntity, TDto>>();
    }
    ...
    {
        // Act
        var instance = new FilterHandler<TStore, TEntity, TDto>(this._repository.Object);
    ...
    {
        FluentActions.Invoking(() => new FilterHandler<TStore, TEntity, TDto>(default(IStoreRepository<TStore, TEntity>))).Should().Throw<ArgumentNullException>().WithParameterName("repository");
    }
