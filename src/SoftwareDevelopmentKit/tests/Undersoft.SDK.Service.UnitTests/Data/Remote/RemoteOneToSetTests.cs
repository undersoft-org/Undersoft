...
/// <summary>
/// Unit tests for the type <see cref="RemoteOneToSet"/>.
/// </summary>
public class RemoteOneToSet_2Tests
{
    private RemoteOneToSet<TOrigin, TTarget> _testClass;
    private IFixture _fixture;
    private Expression<Func<TOrigin, object>> _originkey;
    private Expression<Func<TTarget, object>> _targetkey;

    /// <summary>
    /// Sets up the dependencies required for the tests for <see cref="RemoteOneToSet"/>.
    /// </summary>
    public RemoteOneToSet_2Tests()
    {
        ...
        this._testClass = _fixture.Create<RemoteOneToSet<TOrigin, TTarget>>();
    }
    ...
    {
        // Act
        var instance = new RemoteOneToSet<TOrigin, TTarget>();

        // Assert
        instance.Should().NotBeNull();

        // Act
        instance = new RemoteOneToSet<TOrigin, TTarget>(this._originkey, this._targetkey);
    ...
    {
        FluentActions.Invoking(() => new RemoteOneToSet<TOrigin, TTarget>(default(Expression<Func<TOrigin, object>>), this._targetkey)).Should().Throw<ArgumentNullException>().WithParameterName("originkey");
    }
    ...
    {
        FluentActions.Invoking(() => new RemoteOneToSet<TOrigin, TTarget>(this._originkey, default(Expression<Func<TTarget, object>>))).Should().Throw<ArgumentNullException>().WithParameterName("targetkey");
    }
