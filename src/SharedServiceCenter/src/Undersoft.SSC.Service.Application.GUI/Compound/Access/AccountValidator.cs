    ...
    public AccountValidator(IServicer servicer) : base(servicer)
    {
        ValidationScope(
            ...
            () =>
            {
                ...
                ValidateRequired(p => p.Model.Personal.Country);
                ValidateRequired(p => p.Model.Personal.CityName);
                ValidateRequired(p => p.Model.Personal.Postcode);
                ValidateRequired(p => p.Model.Personal.StreetName);
                ValidateRequired(p => p.Model.Personal.BuildingNumber);
                ValidateRequired(p => p.Model.Personal.ApartmentNumber);
